function [ball] = triaball1(pp,pw,ee)
%TRIABALL1 calc. the ortho-balls associated with a 1-simplex
%triangulation embedded in R^2 or R^3.
%   [BB] = TRIABALL1(PP,PW,TT) returns the set of pwr-balls
...
%---------------------------------------------- basic checks    
    if ( ~isnumeric(pp) || ...
         ~isnumeric(pw) || ...
         ~isnumeric(ee) )
        error('triaball1:incorrectInputClass' , ...
...
%---------------------------------------------- basic checks
    if (ndims(pp) ~= +2 || ...
        ndims(pw) ~= +2 || ...
        ndims(ee) ~= +2 )
        error('triaball1:incorrectDimensions' , ...
    ...
    error('triaball1:unsupportedDimension' , ...
